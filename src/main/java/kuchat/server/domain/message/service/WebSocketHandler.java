...
//            List<WebSocketSession> joinSessions = joinRequest.getMemberIds().stream()
//                    .map(this::getSession).toList();
//            Set<WebSocketSession> chatroomSessionSet = getSessionSet(joinRequest.getChatroomId());
//
//            log.info("[onChatMessageEvent : JOIN] 세션을 추가하기 전 set = {}", chatroomSessions.toString());
//            chatroomSessionSet.addAll(joinSessions);
...
//            WebSocketSession webSocketSession = getSession(chatMessage.getSenderId());
...
//            redisController.deleteChatroomSession(chatMessage.getChatroomId(), chatMessage.getSenderId());
//        }
//
