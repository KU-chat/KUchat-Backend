...
public enum BaseResponse {
    ...
    UNSUPPORTED_TOKEN(1003, HttpStatus.BAD_REQUEST, "지원하지 않는 토큰 형식입니다."),
    INVALID_TOKEN(1004, HttpStatus.BAD_REQUEST, "유효하지 않은 토큰입니다. 다시 로그인 해주세요."),
    EXPIRED_TOKEN(1005, HttpStatus.BAD_REQUEST, "유효기간이 만료된 토큰입니다. 다시 로그인 해주세요."),
    INVALID_SIGNATURE(1006, HttpStatus.BAD_REQUEST, "JWT 토큰의 서명이 유효하지 않습니다."),
    ACCESS_DENIED(1007, HttpStatus.UNAUTHORIZED, "요청을 처리할 권한이 없습니다."),
    ...
    STOMP_ACCESSOR_NULL(4011, HttpStatus.BAD_REQUEST, "받은 요청의 stomp header accessor가 존재하지 않습니다.(null) 다시 시도해주세요."),
    ...
    ALREADY_APPLY(6008, HttpStatus.BAD_REQUEST, "사용자 둘 중 한명이 이미 요청을 보낸 관계입니다."),
    NOT_FOUND_FRIEND(6009, HttpStatus.NOT_FOUND, "둘은 친구가 아니었습니다~~"),
    NOT_FOUND_PLUSID(6010, HttpStatus.NOT_FOUND, "해당 plus id를 사용하는 사용자가 존재하지 않습니다."),
    NOT_FOUND_BLOCK(6011, HttpStatus.NOT_FOUND, "해당 사용자들 사이에 차단 관계가 존재하지 않습니다."),
    BLOCKED_MEMBER(6012, HttpStatus.BAD_REQUEST, "차단하거나 차단 당한 상대의 프로필을 조회할 수 없습니다."),
    ALREADY_FRIEND(6013, HttpStatus.BAD_REQUEST, "이미 친구 관계인 사이에서 친구 관계를 중복하여 맺을 수 없습니다"),
